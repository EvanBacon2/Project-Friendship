...
/*
 * Wraps a variable in a view which allows its value to only be changed through requests
 */
public class RequestableWrapper<T> : PriorityWrapper<T>, IRequestPort<T>, IUniqueRequest<T>, IRequest<T> {
    public RequestableWrapper(Func<T> get, Action<T> set, IPriorityReference reference, IPriorityManager priority) :
